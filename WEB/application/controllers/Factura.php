...
class Factura extends CI_Controller {
		...
		switch ($modulo) {
			case 'usuarios':
				...
				try{
					...
					$params = array("vista" => $vista, "mod" => $modulo, "titulo" => $titulo, 'datos' => $this->Usuarios_model->listar(),  'datos_sucursal' => $this->Usuarios_model, 'accion' => ($accion != null) ? $this->accion : null );
	...
	public function ModificarUsuario($a = null){
		if($_SERVER['REQUEST_METHOD'] == 'POST'){
			...
			$this->Usuarios_model->ModificarUsuario($var);
			$this->res = $this->Usuarios_model->getRes();
		}
